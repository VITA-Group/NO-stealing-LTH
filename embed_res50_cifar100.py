...
a = torch.load("ownership/res50_cifar100_extreme.pth.tar", map_location="cpu")
a.keys()
...
plt.imshow(vis)
plt.savefig(f"ownership/res50_cifar100_vis_{max_name}.pdf")
plt.close()
torch.save(mask, f'ownership/res50_cifar100_qrcode_{max_name}.pth.tar')
...
plt.close()
torch.save(mask, 'ownership/res50_cifar100_extreme.pth.tar')
'''