...
def main():
    ...
    qr.add_data('signature2')
    qr.make(fit=True)
    
    img = qr.make_image(fill_color="black", back_color="white")
    code = np.array(img)
    mask = current_mask['layer2.2.conv1.weight_mask'][3:3+29, 1:1+29].clone()
    original_mask = mask.clone()
    mask_flat = mask.sum((2,3)) > 0
    #mask = code


    for i in range(code.shape[0]):
        for j in range(code.shape[1]):
            if code[i,j] == 1 and mask_flat[i,j] == 0:
                _ = np.array([0] * 9)
                _[0] = 1
                new_mask = np.random.permutation(_)
                mask[i,j] = torch.from_numpy(new_mask.reshape((3, 3)))
                mask_flat[i,j] == 1
            elif code[i,j] == 0 and mask_flat[i,j] == 1:
                mask[i,j] = 0
                mask_flat[i,j] == 0

    mask[0:9, 0:9] = original_mask[0:9, 0:9]
    mask[-9:,:9] = original_mask[-9:,:9]
    mask[:9,-9:] = original_mask[:9,-9:]
    mask[20:25, 20:25] = original_mask[20:25, 20:25]
    mask[-8, 4 * 3 + 9] = 1
    mask[6] = original_mask[6]
    mask[:, 6] = original_mask[:, 6]

    diff = (mask.sum() - current_mask['layer2.2.conv1.weight_mask'].sum()).int().item()
    pos = np.expand_dims((code == 1), (2, 3)) * np.expand_dims(mask_flat == 1, (2,3)) * (mask.numpy() == 0)
    pos = np.where(pos)
    pos = np.stack(pos)
    print(pos.shape)
    print(diff)
    ...
    for i in range(pos.shape[1]):
        p = pos[:, i]
        mask[p[0], p[1], p[2], p[3]] = 1

    current_mask['layer2.2.conv1.weight_mask'][3:3+29, 1:1+29] = mask
    state_dict['layer2.2.conv1.weight_mask'] = current_mask['layer2.2.conv1.weight_mask']
    state_dict['layer2.2.conv1.weight_orig'][3:3+29, 1:1+29] = torch.randn((29, 29, 3, 3)) * 0.1
